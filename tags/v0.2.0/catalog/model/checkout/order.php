...
class ModelCheckoutOrder extends Model {
	...
	public function create($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` (store_id, store_name, store_url, customer_id, customer_group_id, firstname, lastname, email, telephone, fax, total, language_id, currency, currency_id, value, coupon_id, ip, shipping_firstname, shipping_lastname, shipping_company, shipping_address_1, shipping_address_2, shipping_city, shipping_postcode, shipping_zone, shipping_zone_id, shipping_country, shipping_country_id, shipping_address_format, shipping_method, payment_firstname, payment_lastname, payment_company, payment_address_1, payment_address_2, payment_city, payment_postcode, payment_zone, payment_zone_id, payment_country, payment_country_id, payment_address_format, payment_method, comment, date_modified, date_added) VALUES ('" . (int)$data['store_id'] . "', '" . $this->db->escape($data['store_name']) . "', '" . $this->db->escape($data['store_url']) . "', '" . (int)$data['customer_id'] . "', '" . (int)$data['customer_group_id'] . "', '" . $this->db->escape($data['firstname']) . "', '" . $this->db->escape($data['lastname']) . "', '" . $this->db->escape($data['email']) . "', '" . $this->db->escape($data['telephone']) . "', '" . $this->db->escape($data['fax']) . "', '" . (float)$data['total'] . "', '" . (int)$data['language_id'] . "', '" . $this->db->escape($data['currency']) . "', '" . (int)$data['currency_id'] . "', '" . (float)$data['value'] . "', '" . (int)$data['coupon_id'] . "', '" . $this->db->escape($data['ip']) . "', '" . $this->db->escape($data['shipping_firstname']) . "', '" . $this->db->escape($data['shipping_lastname']) . "', '" . $this->db->escape($data['shipping_company']) . "', '" . $this->db->escape($data['shipping_address_1']) . "', '" . $this->db->escape($data['shipping_address_2']) . "', '" . $this->db->escape($data['shipping_city']) . "', '" . $this->db->escape($data['shipping_postcode']) . "', '" . $this->db->escape($data['shipping_zone']) . "', '" . (int)$data['shipping_zone_id'] . "', '" . $this->db->escape($data['shipping_country']) . "', '" . (int)$data['shipping_country_id'] . "', '" . $this->db->escape($data['shipping_address_format']) . "', '" . $this->db->escape($data['shipping_method']) . "', '" . $this->db->escape($data['payment_firstname']) . "', '" . $this->db->escape($data['payment_lastname']) . "', '" . $this->db->escape($data['payment_company']) . "', '" . $this->db->escape($data['payment_address_1']) . "', '" . $this->db->escape($data['payment_address_2']) . "', '" . $this->db->escape($data['payment_city']) . "', '" . $this->db->escape($data['payment_postcode']) . "', '" . $this->db->escape($data['payment_zone']) . "', '" . (int)$data['payment_zone_id'] . "', '" . $this->db->escape($data['payment_country']) . "', '" . (int)$data['payment_country_id'] . "', '" . $this->db->escape($data['payment_address_format']) . "', '" . $this->db->escape($data['payment_method']) . "', '" . $this->db->escape($data['comment']) . "', NOW(), NOW())");

		$order_id = $this->db->getLastId();

		foreach ($data['products'] as $product) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_product (order_id, product_id, name, model, price, total, tax, quantity) VALUES ('" . (int)$order_id . "', '" . (int)$product['product_id'] . "', '" . $this->db->escape($product['name']) . "', '" . $this->db->escape($product['model']) . "', '" . (float)$product['price'] . "', '" . (float)$product['total'] . "', '" . (float)$product['tax'] . "', '" . (int)$product['quantity'] . "')");

			$order_product_id = $this->db->getLastId();

			foreach ($product['option'] as $option) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_option (order_id, order_product_id, product_option_value_id, name, `value`, price, prefix) VALUES ('" . (int)$order_id . "', '" . (int)$order_product_id . "', '" . (int)$option['product_option_value_id'] . "', '" . $this->db->escape($option['name']) . "', '" . $this->db->escape($option['value']) . "', '" . (float)$product['price'] . "', '" . $this->db->escape($option['prefix']) . "')");
			}

			foreach ($product['download'] as $download) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_download (order_id, order_product_id, name, filename, mask, remaining) VALUES ('" . (int)$order_id . "', '" . (int)$order_product_id . "', '" . $this->db->escape($download['name']) . "', '" . $this->db->escape($download['filename']) . "', '" . $this->db->escape($download['mask']) . "', '" . (int)($download['remaining'] * $product['quantity']) . "')");
			}
		}

		foreach ($data['totals'] as $total) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_total (order_id, title, text, `value`, sort_order) VALUES ('" . (int)$order_id . "', '" . $this->db->escape($total['title']) . "', '" . $this->db->escape($total['text']) . "', '" . (float)$total['value'] . "', '" . (int)$total['sort_order'] . "')");
		}
	...
	public function confirm($order_id, $order_status_id, $comment = '') {
		...
		if ($order_query->num_rows) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history (order_id, order_status_id, notify, comment, date_added) VALUES ('" . (int)$order_id . "', '" . (int)$order_status_id . "', '1', '" . $this->db->escape($comment) . "', NOW())");
	...
	public function update($order_id, $order_status_id, $comment = '', $notify = FALSE) {
		...
		if ($order_query->num_rows) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history (order_id, order_status_id, notify, comment, date_added) VALUES ('" . (int)$order_id . "', '" . (int)$order_status_id . "', '" . (int)$notify . "', '" . $this->db->escape($comment) . "', NOW())");
