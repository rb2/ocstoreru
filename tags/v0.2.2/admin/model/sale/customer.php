...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer (firstname, lastname, email, telephone, fax, newsletter, customer_group_id, password, status, date_added) VALUES ('" . $this->db->escape($data['firstname']) . "', '" . $this->db->escape($data['lastname']) . "', '" . $this->db->escape($data['email']) . "', '" . $this->db->escape($data['telephone']) . "', '" . $this->db->escape($data['fax']) . "', '" . (int)$data['newsletter'] . "', '" . (int)$data['customer_group_id'] . "', '" . $this->db->escape(md5($data['password'])) . "', '" . (int)$data['status'] . "', NOW())");

      	$customer_id = $this->db->getLastId();

      	if (isset($data['addresses'])) {
      		foreach ($data['addresses'] as $address) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "address (customer_id, firstname, lastname, company, address_1, address_2, city, postcode, country_id, zone_id) VALUES ('" . (int)$customer_id . "', '" . $this->db->escape($address['firstname']) . "', '" . $this->db->escape($address['lastname']) . "', '" . $this->db->escape($address['company']) . "', '" . $this->db->escape($address['address_1']) . "', '" . $this->db->escape($address['address_2']) . "', '" . $this->db->escape($address['city']) . "', '" . $this->db->escape($address['postcode']) . "', '" . (int)$address['country_id'] . "', '" . (int)$address['zone_id'] . "')");
	...
	public function getCustomers($data = array()) {
		$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS pname, cg.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group cg ON (c.customer_group_id = cg.customer_group_id) ";
