...
class ControllerLocalisationLanguage extends Controller {
	...
	public function enable() {
		$this->changeStatusLanguages(1);
	}
	
	public function disable() {
		$this->changeStatusLanguages(0);
	}
	
	private function changeStatusLanguages($status) {
		$this->load->language('localisation/language');
		$this->load->model('localisation/language');
		
		if (isset($this->request->post['selected']) && $this->user->hasPermission('modify', 'localisation/language')) {
			$this->model_localisation_language->changeStatusLanguages($this->request->post['selected'], $status);
	...
	private function getList() {
		...
		$this->data['enable'] = HTTPS_SERVER . 'index.php?route=localisation/language/enable&token=' . $this->session->data['token'] . $url;
		$this->data['disable'] = HTTPS_SERVER . 'index.php?route=localisation/language/disable&token=' . $this->session->data['token'] . $url;
