...
class ControllerCommonHome extends Controller {
	...
	public function chart() {
		...
		switch ($range) {
			case 'day':
				for ($i = 0; $i < 24; $i++) {
				    $query = $this->db->query("SELECT HOUR(date_added) as hadded, COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND (DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "') GROUP BY 1 ORDER BY 1 ASC");
					...
					$query = $this->db->query("SELECT HOUR(date_added) as hadded, COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "' GROUP BY 1 ORDER BY 1 ASC");
