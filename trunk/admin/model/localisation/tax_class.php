...
class ModelLocalisationTaxClass extends Model {
	public function addTaxClass($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "tax_class (title, description, date_added) VALUES ('" . $this->db->escape($data['title']) . "', '" . $this->db->escape($data['description']) . "', NOW())");

		$tax_class_id = $this->db->getLastId();

		if (isset($data['tax_rate'])) {
			foreach ($data['tax_rate'] as $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "tax_rate (geo_zone_id, tax_class_id, priority, rate, description, date_added) VALUES ('" . (int)$value['geo_zone_id'] . "', '" . (int)$tax_class_id . "', '" . (int)$value['priority'] . "', '" . (float)$value['rate'] . "', '" . $this->db->escape($value['description']) . "', NOW())");
			}
		}
