...
class ModelLocalisationWeightClass extends Model {
	public function addWeightClass($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "weight_class (value) VALUES ('" . (float)$data['value'] . "')");

		$weight_class_id = $this->db->getLastId();

		foreach ($data['weight_class_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "weight_class_description (weight_class_id, language_id, title, unit) VALUES ('" . (int)$weight_class_id . "', '" . (int)$language_id . "', '" . $this->db->escape($value['title']) . "', '" . $this->db->escape($value['unit']) . "')");
		}
