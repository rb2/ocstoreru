...
class ModelSaleCoupon extends Model {
	public function addCoupon($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "coupon (code, discount, type, total, logged, shipping, date_start, date_end, uses_total, uses_customer, status, date_added) VALUES ('" . $this->db->escape($data['code']) . "', '" . (float)$data['discount'] . "', '" . $this->db->escape($data['type']) . "', '" . (float)$data['total'] . "', '" . (int)$data['logged'] . "', '" . (int)$data['shipping'] . "', '" . $this->db->escape($data['date_start']) . "', '" . $this->db->escape($data['date_end']) . "', '" . (int)$data['uses_total'] . "', '" . (int)$data['uses_customer'] . "', '" . (int)$data['status'] . "', NOW())");

      	$coupon_id = $this->db->getLastId();

      	foreach ($data['coupon_description'] as $language_id => $value) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "coupon_description (coupon_id, language_id, name, description ) VALUES ('" . (int)$coupon_id . "', '" . (int)$language_id . "', '" . $this->db->escape($value['name']) . "', '" . $this->db->escape($value['description']) . "')");
      	}
