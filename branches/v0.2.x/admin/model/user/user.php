...
class ModelUserUser extends Model {
	public function addUser($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "user` (username, password, firstname, lastname, email, user_group_id, status, date_added) VALUES ('" . $this->db->escape($data['username']) . "', '" . $this->db->escape(md5($data['password'])) . "', '" . $this->db->escape($data['firstname']) . "', '" . $this->db->escape($data['lastname']) . "', '" . $this->db->escape($data['email']) . "', '" . (int)$data['user_group_id'] . "', '" . (int)$data['status'] . "', NOW())");
	}
